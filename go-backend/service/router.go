...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/organizations", deleteOrganizationHandler(deps)).Methods(http.MethodDelete).Headers(versionHeader, v1)
	router.HandleFunc("/organizations", updateOrganizationHandler(deps)).Methods(http.MethodPut).Headers(versionHeader, v1)
	return
}
